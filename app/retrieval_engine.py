            ...
            for obj_label, filter_spec in norm_object_filters.items():
                ...
                for kf_id, obj_ids in candidate_objects.items():
                    ...
                    if filter_spec.get('type') == 'count_aware':
                        ...
                        # Debug logging
                        logger.info(f"Count-aware processing: {obj_label} detected={detected_count}, required={required_count}")
                        ...
                        if not self._validate_count_constraint(detected_count, required_count):
                            logger.info(f"Count mismatch: {obj_label} count {detected_count} doesn't match requirement {required_count}")
