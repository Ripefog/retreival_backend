        ...
        if user_query:
            ...
            if filtered_users:
                ...
                if expr:
                    ...
                    expr = user_expr
        # return f"({expr}) && (keyframe_id like 'L%')"
        return expr
        ...
        try:
            if kf_ids_to_fetch:
                query_body = {
                    "query": {
                        "terms": {
                            "keyframe_id.keyword": kf_ids_to_fetch
                        }
                    },
                    "_source": ["keyframe_id", "ocr_text"],
                ...
                logger.info(f"Loaded OCR text for {len(ocr_texts_from_es)}/{len(kf_ids_to_fetch)} keyframes")
                
        except Exception as e:
            logger.error(f"Failed to query OCR texts from Elasticsearch: {e}")
            return
