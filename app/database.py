...
class DatabaseManager:
    ...
    async def connect_milvus(self):
        ...
        try:
            ...
            logger.info(
                f"Connecting to Milvus at {params['host']}:{params['port']} (User: {params.get('user', 'N/A')})...")
    ...
    async def connect_elasticsearch(self):
        ...
        try:
            ...
            logger.info(
                f"Connecting to Elasticsearch at {settings.ELASTICSEARCH_HOST}:{settings.ELASTICSEARCH_PORT} (User: {settings.ELASTICSEARCH_USERNAME})...")
            ...
            if self.es_client.ping():
                ...
                for index in indices:
                    try:
                        ...
                        indices_info[index] = {"exists": False, "doc_count": 0}
                return {
                    "status": "connected",
